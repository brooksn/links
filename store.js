...
const storeLoc = './.data/links.json'
const store = fs.existsSync(storeLoc) ? require(storeLoc) : { links: [] }
const matchMost = new RegExp(
  /(?!%E2%9sea%8[cC])(%E2%9C%8C%F0%9F%8F|%F0%\w\w%\w\w%\w\w|%E2%\w\w%\w\w)((%F0%9F%8F)?%BA|(%F0%9F%8F)?%BB|(%F0%9F%8F)?%BC|(%F0%9F%8F)?%BD|(%F0%9F%8F)?%BE|(%F0%9F%8F)?%BF)/,
  'gi'
)
...
const saveLink = (url, slug, isPrivate) => {
  ...
  fs.writeFileSync(storeLoc, JSON.stringify(store, null, '  '), 'utf8')
